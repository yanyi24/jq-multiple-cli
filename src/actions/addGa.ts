...
class Ga {
  ...
  private getFlagHtml() {
    try {
      ...
      console.info(chalk.red(`读取文件${this.pageName}失败`));
    }
  }
  // 获取xlsx文件的代码
  ...
  async reWriteSingleFile() {
    ...
    try {
      ...
      console.info(chalk.green(`文件${this.pageName}添加GA代码成功~`));
    } catch (e) {
      console.info(chalk.red(`重写文件${this.pageName}失败！`));
    }
  }
}

export default function (name: string, option: {dir: string}) {
